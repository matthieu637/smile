...
MCar::State& MCar::step(const DAction& ac)
...
DState* MCar::computeDState() const {
    DState* dst = new DState(stempl, 0);
//     LOG_DEBUG(st.velocity << " "<< round(Utils::transform(st.velocity, -mcar_max_velocity, mcar_max_velocity, 0, 8 )) );
